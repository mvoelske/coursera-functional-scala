...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def timesAcc(chars: List[Char], pairs: List[(Char, Int)]): List[(Char, Int)] =
      (chars, pairs) match {
        case (Nil, acc) => acc
        case (c :: cs, Nil) => timesAcc(cs, (c, 1) :: Nil)
        case (c :: cs, (achr, acnt) :: accs) => if (c == achr)
          timesAcc(cs, (achr, acnt + 1) :: accs)
        else
          timesAcc(cs, (c, 1) :: (achr, acnt) :: accs)

      }
    timesAcc(chars.sorted, Nil)
  }
   ...
   */
  def singleton(trees: List[CodeTree]): Boolean = trees.size == 1 // is that it?!?
